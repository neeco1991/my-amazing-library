import { defineComponent as h, ref as g, inject as N, watch as T, resolveComponent as t, openBlock as f, createElementBlock as b, Fragment as q, createVNode as e, withCtx as n, renderSlot as C, createCommentVNode as R, unref as _, createBlock as x, useSlots as S, provide as X, computed as I, onMounted as L, createElementVNode as D, reactive as Y, normalizeClass as j } from "vue";
const z = "data:image/png;base64,", Z = { key: 0 }, H = /* @__PURE__ */ h({
  __name: "IconFeature",
  props: {
    coordinates: null,
    selected: { type: Boolean },
    icon: null,
    id: null
  },
  setup(o) {
    const l = o, r = g(!1), s = N("ol-format");
    new s.GeoJSON();
    const m = N("ol-selectconditions").pointerMove, p = (u) => {
    }, c = (u) => u.values_.id != null, i = N("selectedFeature");
    return T(
      [i],
      () => {
        i && i.value === l.id ? r.value = !0 : r.value = !1;
      },
      { immediate: !0 }
    ), (u, F) => {
      const d = t("ol-overlay"), v = t("ol-style-icon"), w = t("ol-style"), y = t("ol-interaction-select"), A = t("ol-geom-point"), k = t("ol-feature"), M = t("ol-source-vector"), B = t("ol-vector-layer");
      return f(), b(q, null, [
        e(d, { position: o.coordinates }, {
          default: n(() => [
            r.value ? (f(), b("div", Z, [
              C(u.$slots, "overlay")
            ])) : R("", !0)
          ]),
          _: 3
        }, 8, ["position"]),
        e(y, {
          onSelect: p,
          condition: _(m),
          filter: c
        }, {
          default: n(() => [
            e(w, null, {
              default: n(() => [
                e(v, {
                  src: o.icon || _(z)
                }, null, 8, ["src"])
              ]),
              _: 1
            })
          ]),
          _: 1
        }, 8, ["condition"]),
        e(B, null, {
          default: n(() => [
            e(M, null, {
              default: n(() => [
                e(k, {
                  properties: { id: o.id }
                }, {
                  default: n(() => [
                    e(A, { coordinates: o.coordinates }, null, 8, ["coordinates"])
                  ]),
                  _: 1
                }, 8, ["properties"])
              ]),
              _: 1
            }),
            e(w, null, {
              default: n(() => [
                e(v, {
                  src: o.icon || _(z),
                  scale: 0.75
                }, null, 8, ["src", "scale"])
              ]),
              _: 1
            })
          ]),
          _: 1
        })
      ], 64);
    };
  }
}), E = /* @__PURE__ */ h({
  __name: "LineFeature",
  props: {
    from: null,
    to: null,
    color: null,
    width: null
  },
  setup(o) {
    return (l, r) => {
      const s = t("ol-geom-line-string"), a = t("ol-style-stroke"), m = t("ol-style"), p = t("ol-feature"), c = t("ol-source-vector"), i = t("ol-vector-layer");
      return f(), x(i, null, {
        default: n(() => [
          e(c, null, {
            default: n(() => [
              e(p, null, {
                default: n(() => [
                  e(s, {
                    coordinates: [o.from, o.to]
                  }, null, 8, ["coordinates"]),
                  e(m, null, {
                    default: n(() => [
                      e(a, {
                        color: o.color || "yellow",
                        width: o.width || 5
                      }, null, 8, ["color", "width"])
                    ]),
                    _: 1
                  })
                ]),
                _: 1
              })
            ]),
            _: 1
          })
        ]),
        _: 1
      });
    };
  }
}), U = /* @__PURE__ */ h({
  __name: "Map",
  props: {
    clickAllowed: { type: Boolean, default: !0 },
    center: { default: [40, 40] },
    projection: { default: "EPSG:4326" },
    zoom: { default: 8 },
    rotation: { default: 0 },
    tileUrl: { default: "https://{a-c}.tile.openstreetmap.org/{z}/{x}/{y}.png" }
  },
  setup(o) {
    const l = o, r = S(), s = g(), a = g(null), m = g(!0);
    return X(
      "selectedFeature",
      I(() => a.value)
    ), L(() => {
      if (l.clickAllowed !== !1 && r.default !== void 0) {
        const p = r.default().filter((c) => c.type.__name === "IconFeature");
        s.value.map.on("click", (c) => {
          if (!m.value)
            return;
          const i = c.map.getView().getZoom(), u = Math.pow(2, i), d = p.sort((v, w) => {
            const y = v.props.coordinates, A = w.props.coordinates, k = y[0], M = y[1], B = A[0], W = A[1], O = Math.sqrt(
              Math.pow(k - c.coordinate[0], 2) + Math.pow(M - c.coordinate[1], 2)
            ), P = Math.sqrt(
              Math.pow(B - c.coordinate[0], 2) + Math.pow(W - c.coordinate[1], 2)
            );
            return O - P;
          })[0];
          d !== void 0 && (Math.sqrt(
            // @ts-ignore
            Math.pow(d.props.coordinates[0] - c.coordinate[0], 2) + // @ts-ignore
            Math.pow(d.props.coordinates[1] - c.coordinate[1], 2)
          ) < 40 / u ? a.value = d.props.id : a.value = null);
        });
      }
    }), (p, c) => {
      const i = t("ol-view"), u = t("ol-source-osm"), F = t("ol-tile-layer"), d = t("ol-map");
      return f(), x(d, {
        loadTilesWhileAnimating: !0,
        loadTilesWhileInteracting: !0,
        style: { height: "400px", width: "1000px" },
        ref_key: "olmap",
        ref: s
      }, {
        default: n(() => [
          e(i, {
            ref: "view",
            center: l.center,
            rotation: l.rotation,
            zoom: l.zoom,
            projection: l.projection
          }, null, 8, ["center", "rotation", "zoom", "projection"]),
          e(F, null, {
            default: n(() => [
              e(u, {
                url: l.tileUrl
              }, null, 8, ["url"])
            ]),
            _: 1
          }),
          C(p.$slots, "default", { test: 1 })
        ]),
        _: 3
      }, 512);
    };
  }
}), G = /* @__PURE__ */ D("div", null, "F1", -1), K = /* @__PURE__ */ D("div", null, "F2", -1), V = /* @__PURE__ */ h({
  __name: "HelloWorld",
  setup(o) {
    return (l, r) => (f(), x(U, {
      center: [40, 40],
      rotation: 0,
      zoom: 8,
      projection: "EPSG:4326"
    }, {
      default: n(() => [
        e(H, {
          icon: _(z),
          coordinates: [40, 40],
          id: "f1"
        }, {
          overlay: n(() => [
            G
          ]),
          _: 1
        }, 8, ["icon"]),
        e(H, {
          icon: _(z),
          coordinates: [41, 41],
          id: "f2"
        }, {
          overlay: n(() => [
            K
          ]),
          _: 1
        }, 8, ["icon"]),
        e(E, {
          from: [40, 40],
          to: [41, 41]
        })
      ]),
      _: 1
    }));
  }
}), Q = /* @__PURE__ */ h({
  __name: "MyButton",
  props: {
    primary: { type: Boolean, default: !1 }
  },
  setup(o) {
    const l = o, { primary: r } = Y(l);
    return (s, a) => (f(), b("button", {
      class: j(["btn", { primary: _(r) }])
    }, [
      C(s.$slots, "default", {}, void 0, !0)
    ], 2));
  }
});
const J = (o, l) => {
  const r = o.__vccOpts || o;
  for (const [s, a] of l)
    r[s] = a;
  return r;
}, $ = /* @__PURE__ */ J(Q, [["__scopeId", "data-v-24f788ad"]]), eo = {
  install: (o) => {
    o.component("HelloWorld", V), o.component("MyButton", $), o.component("wfc-icon", H), o.component("wfc-line", E), o.component("wfc-map", U);
  }
};
export {
  V as HelloWorld,
  H as IconFeature,
  E as LineFeature,
  U as Map,
  $ as MyButton,
  eo as default
};
